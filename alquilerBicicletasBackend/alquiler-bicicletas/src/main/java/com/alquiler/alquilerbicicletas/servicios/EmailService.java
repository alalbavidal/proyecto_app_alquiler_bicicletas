...
public class EmailService {
    ...
    @Value("${app.email.sender.name:Alquilando Pedales}")
    ...
    private void enviarEmailCliente(Reserva reserva, String contratoFilePath) {
        try {
            ...
            String asunto = String.format("Confirmación de Reserva #%d - Alquilando Pedales", reserva.getId());
            helper.setSubject(asunto);

            message.setHeader("X-Priority", "1");
            message.setHeader("X-Mailer", "AlquilandoPedalesApp");
    ...
    private String generarContenidoClienteBonito(Reserva reserva, boolean incluirCancelacion, boolean forzarEnlace) {
        ...
        html.append("<p style='margin:10px 0 0 0;opacity:0.9;'>Alquilando Pedales - Alquiler y Rutas</p>\n");
        html.append("</div>\n");
        ...
        html.append("<p>¡Esperamos que disfrutes de tu experiencia con Alquilando Pedales! 🚴‍♂️</p>\n");
        html.append("<p>Saludos cordiales,<br><strong>El equipo de Alquilando Pedales</strong></p>\n");
        html.append("</div>\n");
        html.append("<div class='footer'>\n");
        html.append("<p><strong>Alquilando Pedales SL</strong></p>\n");
        html.append("<p>Avenida de la Innovación, 14, 41020 Sevilla</p>\n");
        html.append("<p>📞 +34 954 000 111 | ✉️ contacto@alquilandopedales.es</p>\n");
        html.append("<p>© 2026 Alquilando Pedales - Todos los derechos reservados</p>\n");
        html.append("</div>\n");
        html.append("</div>\n");
        html.append("</body>\n");
        html.append("</html>\n");
    ...
    private String generarContenidoEmpresaFuncional(Reserva reserva) {
        ...
        <body>
            <div class="container">
                ...
                <div style="text-align: center; margin-top: 20px; color: #6c757d; font-size: 12px; padding: 15px; background: #e9ecef; border-radius: 5px;">
                    ...
                    <p>Notificación automática - Alquilando Pedales Admin</p>
                    <p>📧 Recibido el: %s</p>
                </div>
            </div>
        </body>
        ...
        """.formatted(
                ...
                "http://localhost:4200/admin/reservas", 
    ...
    private void enviarEmailClienteFallback(Reserva reserva) {
        try {
            ...
            helper.setFrom(remitente, senderName);
            helper.setTo(reserva.getCliente().getEmail());
            helper.setSubject(String.format("Confirmación de Reserva #%d - Alquilando Pedales", reserva.getId()));
    ...
    private boolean adjuntarContratoPDF(MimeMessageHelper helper, Reserva reserva, String contratoFilePath) {
        try {
            ...
            String nombreArchivo = String.format("Contrato_Reserva_%d_AlquilandoPedales.pdf", reserva.getId());
